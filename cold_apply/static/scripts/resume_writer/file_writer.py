...
class PDF(FPDF):
    ...
    def add_skills_section(self, skills):
        ...
        self.cell(w=190, h=5, txt="Professional Skills", ln=1, align="C")

        for i, skill in enumerate(skills):
            # if exp_counter == 4 and exp_counter != exp_count:
            #     self.add_page()
            self.set_font("Times", "B", size=11)
            self.cell(w=100, h=5, txt=skill["obj"].title, ln=0, align="L")
            self.ln(6)
            for bullet in skill["bullets"]:
                ...
                self.set_font("Times", size=11)
                self.multi_cell(w=175, h=5, txt=bullet.bullet.text, border=0, align="J")
                self.ln(1)
            self.ln(4)
...
def write_skills_resume(participant, overview, education, job, skills, sections):
    ...
    education: Education
    job: Job
    experiences: [{'obj': Skill, 'bullets': [WeightedBullets]}]
    """
...
def write_chronological_resume(
    ...
    education: Education
    job: Job
    experiences: [{'obj': Experience, 'bullets': [WeightedBullets]}]
    """
...
def write_resume(context):
    ...
    pdf.output(
        f'{pdf.output_path}{context["job"].company.name}_{context["title"]}.pdf', "F"
    )
